...
const gvetFormSchema = new mongoose_1.Schema({
    name: {
        type: String,
    ...
    currentClass: {
        ...
        enum: ['12th appearing', '12th passed']
    },
    examCenter: {
        type: String,
...
gvetFormSchema.index({ email: 1, createdAt: -1 });
gvetFormSchema.index({ phone: 1, createdAt: -1 });
gvetFormSchema.index({ status: 1, createdAt: -1 });
...
gvetFormSchema.pre('save', function (next) {
    // Clean phone number
    if (this.phone) {
    ...
    if (!this.registrationNumber && this.isNew) {
        ...
        this.registrationNumber = `GVET${year}${randomNum}`;
    }
    next();
});
exports.default = mongoose_1.default.model('GVETForm', gvetFormSchema);
//# sourceMappingURL=GVETForm.js.map